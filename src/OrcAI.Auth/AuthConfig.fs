...
//
// Location: ~/.config/orcai/auth.json
//
// Schema:
//   {
//     "active": "my-app",
...
let readConfigFrom (homeDir: string) : Result<AuthConfigFile, string> =
    try
        ...
        if not (File.Exists(path)) then
            Error "No auth config found. Run 'orcai auth pat --token <tok>' or 'orcai auth app ...' first."
        else
...
/// Delete the legacy ~/.config/orcai/app.pem if it exists.
...
let removeOldPem () : unit =
    try
        let home =
            ...
            | h -> h
        let path = Path.Combine(home, ".config", "orcai", "app.pem")
